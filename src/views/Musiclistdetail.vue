  ...
  <div v-if="complete&&dataok" class="router_container">
    <div class="list_header">
      ...
      <div class="list_info">
        ...
        <div class="owner">
          <myitem :shadow="false" :hoveranim="false" :routemode="true" :router="{name:'userprofile',params:{id:musiclistinfo.creator.userId}}" :imgurl="`${musiclistinfo.creator.avatarUrl}?param=24y24`" class="avatar"/>
          <mytitle :routemode="true" :router="{name:'userprofile',params:{id:musiclistinfo.creator.userId}}" class="username">{{musiclistinfo.creator.nickname}}</mytitle>
        ...
        <div class="button_group">
          ...
          <el-button v-if="increatelist&&incollectlist" type="info" size="small" icon="FolderAdd" @click="clickcollectit(musiclistinfo,1)" plain round>收藏</el-button>
          <el-button v-else-if="increatelist" type="info" size="small" icon="FolderChecked" @click="clickcollectit(musiclistinfo,2)" plain round>已收藏</el-button>
          <el-button @click="addall" type="info" size="small" icon="Plus" plain round>添加全部至播放列表</el-button>
        </div>
        <div class="detail">歌曲:{{musiclistinfo.trackCount}} 播放:<playnumber :number="musiclistinfo.playCount" /></div>
        <el-tooltip
          v-if="musiclistinfo.description"
          class="box-item"
          effect="dark"
          :content="musiclistinfo.description"
          placement="bottom"
        >
    ...
    >
        ...
        <el-table-column show-overflow-tooltip label="标题" min-width="160">
            <template v-slot="{row}">
              <span class="text_title" :class="{'playon':nowplaymusic&&row.id===nowplaymusic.id}">{{row.name}}<span v-if="row.alia[0]" class="sub_info">({{row.alia[0]}})</span></span><tags class="row_tag" v-if="row.fee===1">VIP</tags><tags  class="row_tag" v-if="row.fee===8||row.fee===1">SQ</tags><tags  class="row_tag" v-if="row.mv!=0" :routemode="true" :router="{name:'videodetail',params:{id:row.mv,type:'mv'}}">MV</tags>
            </template>
        </el-table-column>
        ...
        <el-table-column show-overflow-tooltip label="专辑" min-width="60">
            <template v-slot="{row}">
            <mytitle size="small" color="light" :class="{'playon':nowplaymusic&&row.id===nowplaymusic.id}" :routemode="true" :router="{name:'albumsdetail',params:{id:row.al.id}}">{{row.al.name}}</mytitle>
            </template>
        </el-table-column>
        ...
        <el-tab-pane lazy label="收藏者" name="collector">
            <div v-if="musiclistsubsribers.length" class="collector">
              <div class="user" v-for="(item,index) in musiclistsubsribers" :key="index">
                <myitem :hoveranim="false" :shadow="false" :routemode="true" :router="{name:'userprofile',params:{id:item.userId}}" :imgurl="`${item.avatarUrl}?param=90y90`" class="avater"></myitem>
                <div class="info">
                  <mytitle  :routemode="true" :router="{name:'userprofile',params:{id:item.userId}}">{{item.nickname}}</mytitle>
                  <svg-icon v-if="item.gender===1" class="gender male" iconClass="bx-male-sign"/><svg-icon class="gender female" v-else-if="item.gender===2" iconClass="bx-female-sign"/>
                </div>
              </div>
            </div>
            <el-empty v-else description="没有找到数据"></el-empty>
              <el-pagination
                  ...
                  :total="totalsub"
                  :hide-on-single-page="true"
                  v-model:current-page="currentsubpage"
                  @current-change="currentsubchange"
                  class="pagination"
                />
        </el-tab-pane>
    </el-tabs>
