    ...
    <div v-if="complete && dataok" class="router_container">
      <div class="list_header">
        ...
        <div class="list_info">
          <div class="title">
            ...
            <svg-icon v-if="userinfo.profile.gender === 1" class="gender male" iconClass="bx-male-sign" /><svg-icon
              class="gender female" v-else-if="userinfo.profile.gender === 2" iconClass="bx-female-sign" />
          ...
          <div v-if="userid != userinfo.profile.userId">
            <el-button v-if="followed" @click="handlefollowe(userinfo.profile.userId, 0)" class="button_userprofile"
              type="danger" round icon="CloseBold">已关注</el-button>
            <el-button v-else @click="handlefollowe(userinfo.profile.userId, 1)" class="button_userprofile" type="danger"
              round plain icon="Plus">关注</el-button>
          </div>
          <el-tooltip v-if="userinfo.profile.description" class="box-item" effect="dark"
            :content="userinfo.profile.description" placement="bottom">
      ...
      <el-tabs v-model="activeName" tab-position="top" class="el-tabs">
        <el-tab-pane lazy label="创建的歌单" name="createlist">
          <div v-if="musiclistlist.length" class="recommend_list">
            <div class="music_item" v-for="(item, index) in musiclistlist" :key="index">
              <Myitem :imgurl="`${item.coverImgUrl}?param=150y150`" :routemode="true"
        ...
        <el-tab-pane lazy label="创建的播客" name="createradiolist" class="tabs_list">
          <el-table v-if="audiolist.length" :show-header="false" :data="audiolist"
            ...
            <el-table-column show-overflow-tooltip min-width="120">
              <template v-slot="{ row }">
                <mytitle :routemode="true" :router="{ name: 'radiodetail', params: { id: row.id } }">{{ row.name }}<tags
                    class="row_tag" v-if="row.secondCategory">{{ row.secondCategory }}</tags>
                </mytitle>
              </template>
            </el-table-column>
            <el-table-column show-overflow-tooltip min-width="60">
              <template v-slot="{ row }">
                <span class="text_table">声音
                  <playnumber :number="row.programCount" />
                </span>
              </template>
            </el-table-column>
            <el-table-column show-overflow-tooltip min-width="60">
              <template v-slot="{ row }">
                <span class="text_table">收藏
                  <playnumber :number="row.subCount" />
                </span>
              </template>
            </el-table-column>
          </el-table>
