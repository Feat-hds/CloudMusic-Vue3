<template>
    <el-scrollbar>
        ...
        <div v-if="complete && dataok" class="recommend_container">
            <!-- 首页轮播图 -->
            <el-carousel :interval="5000" class="el-carousel" type="card" indicator-position="outside" trigger="click"
                height='200px'>
            ...
            <part title="推荐歌单" :routemode="true" :router="{ path: '/discovermusic/musiclist' }">
                <div class="recommend_list">
                    <div v-if="islogin" class="music_item">
                        <Myitem :routemode="true" :router="{ name: 'dailyrecommend' }" :shadow="false" type="dailyrecommend"
                            class="item"></Myitem>
                    ...
                    <div class="music_item" v-for="(item, index) in personalized" :key="index">
                        <Myitem v-if="index < 9 || !islogin" :imgurl="`${item.picUrl}?param=150y150`" :routemode="true"
                            :router="{ name: 'musiclistdetail', params: { id: item.id } }" :number="item.playCount" class="item">
                        </Myitem>
                        <Mytitle v-if="index < 9 || !islogin" :routemode="true"
                            :router="{ name: 'musiclistdetail', params: { id: item.id } }" class="title">{{ item.name }}</Mytitle>
                    </div>
                </div>
            ...
            <part title="最新音乐" :routemode="true" :router="{ name: 'topsong' }">
                <div class="newest_music">
                    <div @dblclick="addmusicandplay(item)" class="music_item" v-for="(item, index) in personalizednewsong"
                        :key="index">
                        <Myitem @click="addmusicandplay(item)" :shadow="false" :imgurl="`${item.picUrl}?param=50y50`"
                            :id="item.id" class="item"></Myitem>
                        <div class="text_container">
                            <Mytitle hover="default" class="title">{{ item.name }}</Mytitle>
                            <div class="sub">
                                <Mytitle v-for="(artist, index_a) in item.song.artists" :key="index_a" :routemode="true"
                                    :router="{ name: 'artistdetail', params: { id: artist.id } }" class="subtitle" size="small"
                                    color="light"><span class="seg" v-if="index_a > 0">/</span>{{ artist.name }}</Mytitle>
                            </div>
                        </div>
            ...
            <part title="热门播客" :routemode="true" :router="{ name: 'radio' }">
                <div class="hot_player">
                    <div class="music_item" :mainTrackid="item.mainTrackId" v-for="(item, index) in programlist"
                        :key="index">
                        <Myitem :router="{ name: 'radiodetail', params: { id: item.radio.id } }" :routemode="true" :shadow="false"
                            :imgurl="`${item.coverUrl}?param=80y80`" class="item"></Myitem>
                        <div class="text_container">
                            <Mytitle hover="default" class="title">{{ item.name }}</Mytitle>
                            <tags class="tag" color="gray">{{ item.radio.secondCategory }}</tags>
                            <div class="detail">
                                <Mytitle :router="{ name: 'radiodetail', params: { id: item.radio.id } }" :routemode="true"
                                    class="artist" size="small" color="light">{{ item.radio.name }}</Mytitle>
            ...
            <part title="独家放松">
                <div class="exclusive_mv">
                    <div class="music_item" v-for="(item, index) in exclusivemvlist" :key="index">
                        <Myitem :shadow="false" :imgurl="`${item.picUrl}?param=300y140`" :routemode="true"
                            :router="{ name: 'videodetail', params: { type: 'mv', id: item.id } }" class="item"></Myitem>
            ...
            <part title="推荐MV" :routemode="true" :router="{ path: '/video/mv' }">
                <div class="recommend_mv">
                    <div class="music_item" v-for="(item, index) in mvlist" :key="index">
                        <Myitem :routemode="true" :imgurl="`${item.picUrl}?param=300y140`"
                            :router="{ name: 'videodetail', params: { type: 'mv', id: item.id } }" :number="item.playCount"
                            class="item"></Myitem>
                        <Mytitle :routemode="true" :router="{ name: 'videodetail', params: { type: 'mv', id: item.id } }"
                            class="title">{{ item.name }}</Mytitle>
                        <Mytitle :routemode="true" :router="{ name: 'artistdetail', params: { id: item.artistId } }" class="title"
                            size="small" color="light">{{ item.artistName }}</Mytitle>
                    </div>
                </div>
            </part>
        </div>
