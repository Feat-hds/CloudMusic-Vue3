            ...
            <part class="type">
                <div class="category_container">
                    <el-popover placement="bottom-start" :width="700" trigger="click">
                        ...
                        <div class="all_type">
                            <span @click="handleClick({ name: '全部', id: 0 })" class="all_type_sort_child"
                                :class="{ 'is-active': currentsort.id === 0 }">全部分类</span>
                        </div>
                        <div class="pop_container_musiclist">
                            <div class="pop_container_musiclist_sort_container" v-for="(item, index) in allcat"
                                :key="index">
            ...
            <part class="music_list_container">
                <div class="music_list">
                    <div class="item_container" v-for="(item, index) in playlist" :key="index">
                        <Myitem :creator="item.creator" :imgurl="item.coverImgUrl" :routemode="true"
                            :router="{ name: 'musiclistdetail', params: { id: item.id } }" class="item"
                            :number="item.playCount">
                        </Myitem>
                        <Mytitle :routemode="true" :router="{ name: 'musiclistdetail', params: { id: item.id } }"
                            class="title">
                            {{ item.name }}</Mytitle>
                    </div>
                </div>
                <el-pagination :page-size="50" layout="prev, pager, next" :pager-count="7" :total="total"
