<template>
    ...
    <div v-if="complete&&dataok">
        <div class="group">
            <div class="hotcat_container">
                <div  v-for="(item,index) in catlistarea" :key="index" class='sort_container'>
                    <span @click="handleClick(item)" class="sort_child" :class="{'is-active':item.id===currentmusicarea}">{{item.name}}</span>
                </div>
            </div>
        ...
        <el-table-column width="80">
            <template v-slot="{row}">
                <Myitem @click="addmusicandplay(row)" :shadow="false" :imgurl="`${row.album.picUrl}?param=60y60`" class="cover"></Myitem>
            </template>
        </el-table-column>
        <el-table-column prop="name" show-overflow-tooltip min-width="120">
            <template v-slot="{row}">
                <span @click="addmusicandplay(row)" class="title_video">{{row.name}}<span v-if="row.alias[0]" class="sub_info">({{row.alias[0]}})</span></span><tags class="row_tag" v-if="row.fee===1">VIP</tags><tags  class="row_tag" v-if="row.fee===8||row.fee===1">SQ</tags><tags  class="row_tag" v-if="row.mvid!=0" :routemode="true" :router="{name:'videodetail',params:{id:row.mvid,type:'mv'}}">MV</tags>
            </template>
        </el-table-column>
        <el-table-column show-overflow-tooltip min-width="60">
            <template v-slot="{row}">
            <mytitle size="small" color="light" :routemode="true" :router="{name:'artistdetail',params:{id:item.id}}" v-for="(item,index) in row.artists" :key="index" class="text" >{{item.name}}</mytitle>
            </template>
        </el-table-column>
        <el-table-column show-overflow-tooltip min-width="60">
            <template v-slot="{row}">
            <mytitle size="small" color="light" :routemode="true" :router="{name:'albumsdetail',params:{id:row.album.id}}" class="album">{{row.album.name}}</mytitle>
            </template>
        </el-table-column>
