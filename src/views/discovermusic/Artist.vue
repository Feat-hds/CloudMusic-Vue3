                    ...
                    <div class="hotcat_container">
                        ...
                        <div v-for="(item, index) in catlistinitial" :key="index" class='sort_container'>
                            <span @click="handleClick(item, 3)" class="sort_child"
                                :class="{ 'is-active': item.id === currentinitial }">{{ item.name }}</span>
                        </div>
                    </div>
                </div>
            </part>
            <part>
                <div class="list">
                    <div v-for="(item, index) in artistlist" class="item_container" :key="index">
                        <myitem :shadow="false" :routemode="true"
                            :router="{ name: 'artistdetail', params: { id: item.id } }"
                            :imgurl="`${item.picUrl}?param=150y150`" class="item"></myitem>
                        <Mytitle :routemode="true" :router="{ name: 'artistdetail', params: { id: item.id } }"
                            class="title">
                            {{ item.name }}</Mytitle>
                    </div>
                </div>
