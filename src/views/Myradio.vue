        ...
        <part class="part" title="我的播客" :issub="true" :subtitle="`(${total})`">
            ...
            >
                <el-table-column width="80">
                    <template v-slot="{row}">
                        <Myitem :shadow="false" :routemode="true" :router="{name:'radiodetail',params:{id:row.id}}" :imgurl="`${row.picUrl}?param=60y60`" class="cover"></Myitem>
                    </template>
                </el-table-column>
                ...
                <template v-slot="{row}">
                    <mytitle :routemode="true" :router="{name:'userprofile',params:{id:row.dj.userId}}" color="light" size="small" >by {{row.dj.nickname}}</mytitle>
                </template>
                </el-table-column>
            ...
            <div class="artist_list">
                <div v-for="(item,index) in djrecommend" class="item_container" :key="index">
                    ...
                    <Mytitle :routemode="true" :router="{name:'radiodetail',params:{id:item.id}}" class="radio">{{item.name}}</Mytitle>
                </div>
            </div>
        </part>
        </el-scrollbar>
...
export default {
    ...
    setup(){
        ...
        //     // console.log(result);
        //     if(result.code===200){
        //         subradiolist.value=result.djRadios;
        //         total.value=result.count;
        //         return true;
        //     }
        //     else return Promise.reject();
