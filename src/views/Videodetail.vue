            ...
            <div class="left">
                <part :title="currenttype==='video'?'视频详情':'MV详情'">
                        <video
                            ...
                            height="369px"
                            :poster="
                            route.params.type === 'mv' ? videodetail.cover : videodetail.coverUrl
                            "
                        >
                        </video>
                </part>
                <part>
                    <div class="videoinfo">
                        <div v-if="currenttype==='video'" class="creator">
                            <div class="userinfo">
                                <myitem :hoveranim="false" :shadow="false" :routemode="true" :router="{name:'userprofile',params:{id:videodetail.creator.userId}}" :imgurl="`${videodetail.creator.avatarUrl}?param=60y60`" class="avatar"/>
                                <mytitle :routemode="true" :router="{name:'userprofile',params:{id:videodetail.creator.userId}}" class="user">{{videodetail.creator.nickname}}</mytitle>
                            </div>
                            <el-button v-if="followed" @click="handlefollowvideo(videodetail.creator.userId,0)" class="button_userprofile" type="danger" round icon="CloseBold">已关注</el-button>
                            <el-button v-else @click="handlefollowvideo(videodetail.creator.userId,1)" class="button_userprofile" type="danger" round plain icon="Plus">关注</el-button>
                        </div>
                        <div v-else-if="currenttype==='mv'" class="creator">
                            <div class="userinfo">
                                <myitem :hoveranim="false" :shadow="false" :routemode="true" :router="{name:'artistdetail',params:{id:videodetail.artists[0].id}}" :imgurl="`${videodetail.artists[0].img1v1Url}?param=60y60`" class="avatar"/>
                                <div class="user">
                                <mytitle v-for="(ar,i) in videodetail.artists" :key="i" :routemode="true" :router="{name:'artistdetail',params:{id:ar.id}}">{{i>0?'/':''}}{{ar.name}}</mytitle>
                                </div>
                            </div>
                        </div>
                        <div v-if="currenttype==='video'&&mvlikeinfo" class="video_main">
                            ...
                            <div class="time_number">
                                <span class="video_text">发布:<timetransform :time="videodetail.publishTime"/></span>
                                <span class="video_text">播放:<playnumber :number="videodetail.playTime" /></span>
                            </div>
                            <div class="tags_container">
                                <tags class="tag_item" v-for="(item,index) in videodetail.videoGroup" :key="index" color="gray">{{item.name}}</tags>
                            </div>
                            <div v-if="videodetail.description" class="desc">
                                {{videodetail.description}}
                            </div>
                            <div class="button_group">
                                <el-button v-if="mvlikeinfo.liked"  type="danger" @click="unlikevideo" round icon="StarFilled">赞(<playnumber :number="mvlikeinfo.likedCount"/>)</el-button>
                                <el-button v-else  type="danger" @click="likevideo" plain round icon="Star">赞(<playnumber :number="mvlikeinfo.likedCount"/>)</el-button>
                                <el-button type="danger" @click="uncollectvideo" v-if="subed" round icon="FolderChecked">已收藏(<playnumber :number="videodetail.subscribeCount" />)</el-button>
                                <el-button type="danger" @click="collectvideo" v-else plain round icon="FolderAdd">收藏(<playnumber :number="videodetail.subscribeCount" />)</el-button>
                            </div>
                        </div>
                        <div v-else-if="currenttype==='mv'&&mvlikeinfo" class="video_main">
                            <span class="video_title">{{videodetail.name}}</span>
                            <div class="time_number">
                                <span v-if="currenttype==='video'" class="video_text">发布:<timetransform :time="videodetail.publishTime"/></span>
                                <span v-else-if="currenttype==='mv'" class="video_text">发布:{{videodetail.publishTime}}</span>
                                <span class="video_text video_play">播放:<playnumber :number="videodetail.playCount" /></span>
                            </div>
                            <div v-if="videodetail.desc" class="desc">
                                {{videodetail.desc}}
                            </div>
                            <div class="button_group">
                                <el-button v-if="mvlikeinfo.liked"  type="danger" @click="unlikemv" round icon="StarFilled">赞(<playnumber :number="mvlikeinfo.likedCount"/>)</el-button>
                                <el-button v-else  type="danger" @click="likemv" plain round icon="Star">赞(<playnumber :number="mvlikeinfo.likedCount"/>)</el-button>
                                <el-button type="danger" @click="uncollectmv" v-if="subed" round icon="FolderChecked">已收藏(<playnumber :number="videodetail.subCount" />)</el-button>
                                <el-button type="danger" @click="collectmv" v-else plain round icon="FolderAdd">收藏(<playnumber :number="videodetail.subCount" />)</el-button>
                            </div>
                        </div>
                    </div>
                </part>
            ...
            <part v-if="relatedvideolist.length" title="相关推荐">
                <div v-if="currenttype==='video'" class="recommendlist">
                    <div class="item_container" v-for="(item,index) in relatedvideolist" :key="index">
                        <myitem :number="item.playTime" :duration="item.durationms" :imgurl="`${item.coverUrl}?param=130y70`" :routemode="true" :router="{name:'videodetail',params:{type:'video',id:item.vid}}" class="item" />
                        <div class="info">
                            <mytitle :routemode="true" :router="{name:'videodetail',params:{type:'video',id:item.vid}}" class="title">{{item.title}}</mytitle>
                            <mytitle :routemode="true" :router="{name:'userprofile',params:{id:item.creator[0].userId}}" color="light" size="small"  class="artist">by: {{item.creator[0].userName}}</mytitle>
                        </div>
                    </div>
                </div>
                <div v-else-if="currenttype==='mv'" class="recommendlist">
                    <div class="item_container" v-for="(item,index) in relatedvideolist" :key="index">
                        <myitem :number="item.playCount" :duration="item.duration" :imgurl="`${item.cover}?param=130y70`" :routemode="true" :router="{name:'videodetail',params:{type:'mv',id:item.id}}" class="item" />
                        <div class="info">
                            <mytitle :routemode="true" :router="{name:'videodetail',params:{type:'mv',id:item.id}}" class="title">{{item.name}}</mytitle>
                            <mytitle :routemode="true" :router="{name:'artistdetail',params:{id:item.artists[0].id}}" color="light" size="small"  class="artist">by: {{item.artists[0].name}}</mytitle>
                        </div>
                    </div>
                </div>
            </part>
