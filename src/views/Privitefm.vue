    ...
    <div class="musicplayer">
        ...
        <el-scrollbar v-if="complete && dataok" class="scroll">
            <div class="content">
                <div class="player">
                    <div class="left_warp">
                        <myitem :shadow="false" v-if="nowplayfm" @click="clickplay"
                            :imgurl="nowplayfm.album ? `${nowplayfm.album.picUrl}?param=300y300` : imgnull" class="cover">
                        </myitem>
                        <div class="button_warp">
                            <svg-icon v-if="!like" @click="clicklikeit(nowplayfm.id, true)" iconClass="bx-heart"
                                class="icon"></svg-icon>
                            <svg-icon v-else-if="like" @click="clicklikeit(nowplayfm.id, false)" iconClass="bxs-heart"
                                class="icon like"></svg-icon>
                    ...
                    <div class="right_warp">
                        ...
                        <div v-if="nowplayfm" class="info_music">
                            <div class="artist">
                                <span v-for="(artists, i) in nowplayfm.artists" :key="i">{{ i > 0 ? '/' : '' }}{{
                                    artists.name }}</span>
                            </div>
                        ...
                        <el-scrollbar ref="lyric_ref" class="scroll_lrc">
                            <div class="lyric">
                                <span @click="moveto(index)" class="row_lrc"
                                    :class="{ 'current_lrc': current_lrc === index }" v-for="(lrc, index) in ms"
                                    :key="index">{{ lrc.c }}</span>
...
import { computed, onActivated, onBeforeMount, onDeactivated, watch } from '@vue/runtime-core';
