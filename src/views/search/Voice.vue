<template>
  ...
  <div v-if="complete && dataok">
    ...
    <el-table v-if="voicelist" :show-header="false" :data="voicelist"
      style="width:100%;cursor:default;border-top-style:solid;border-top-color:#BBBBBB;border-top-width:1px;" stripe lazy
      class="list" @row-dblclick="adddjandplay">
      <el-table-column width="80">
        <template v-slot="{ row }">
          <Myitem @click="adddjandplay" :shadow="false" :imgurl="`${row.baseInfo.coverUrl}?param=60y60`" class="cover">
          </Myitem>
        </template>
      </el-table-column>
      ...
      <el-table-column show-overflow-tooltip min-width="60">
        <template v-slot="{ row }">
          <div class="text_table"><svg-icon iconClass="bx-play" />
            <playnumber :number="row.baseInfo.adjustedPlayCount" />
          </div>
        </template>
      </el-table-column>
      <el-table-column show-overflow-tooltip min-width="60">
      ...
      <el-table-column show-overflow-tooltip min-width="60">
        <template v-slot="{ row }">
          <mytitle size="small" color="light" :routemode="true"
            :router="{ name: 'userprofile', params: { id: row.baseInfo.userId } }">by:{{ row.baseInfo.radio.name }}
          </mytitle>
        </template>
      </el-table-column>
