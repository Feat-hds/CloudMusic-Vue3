<template>
    ...
    <el-table
        ...
        <el-table-column show-overflow-tooltip label="标题" min-width="160">
            <template v-slot="{row}">
              <span class="text_title" :class="{'playon':nowplaymusic&&row.id===nowplaymusic.id}" @click="test">{{row.name}}<span v-if="row.alia[0]" class="sub_info">({{row.alia[0]}})</span></span><tags class="row_tag" v-if="row.fee===1">VIP</tags><tags  class="row_tag" v-if="row.fee===8||row.fee===1">SQ</tags><tags  class="row_tag" v-if="row.mvid!=0" :routemode="true" :router="{name:'videodetail',params:{id:row.mv,type:'mv'}}">MV</tags>
            </template>
        </el-table-column>
...
import {reqSearch,reqLikeit,reqSearchbrief} from '@/api/music'
