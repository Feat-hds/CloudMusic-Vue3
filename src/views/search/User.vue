  ...
  <el-table
          ...
          >
            <el-table-column width="80">
              <template v-slot="{row}">
              <Myitem :shadow="false" :imgurl="`${row.avatarUrl}?param=60y60`" :routemode="true" :router="{name:'userprofile',params:{id:row.userId}}" class="cover"></Myitem>
              </template>
            </el-table-column>
            <el-table-column show-overflow-tooltip min-width="120">
              <template v-slot="{row}">
                <mytitle :routemode="true" :router="{name:'userprofile',params:{id:row.userId}}" >{{row.nickname}}<svg-icon v-if="row.gender===1" class="gender male" iconClass="bx-male-sign"/><svg-icon class="gender female" v-else-if="row.gender===2" iconClass="bx-female-sign"/></mytitle>
              </template>
            </el-table-column>
          </el-table>
...
export default {
    ...
    setup(){
      ...
      async function getsearchuser(keyword,limit,offset){
        ...
        //         await getsearchuser(route.query.keyword,30,0);
        //       }
