    ...
    <div class="list_header">
      ...
      <div class="list_info">
        ...
        <div class="button_group">
          <el-button v-if="issub" @click="subartist(artistinfo,2)" type="info" size="small" icon="FolderChecked" plain round>已收藏</el-button>
          <el-button v-else @click="subartist(artistinfo,1)" type="info" size="small" icon="FolderAdd" plain round>收藏</el-button>
        </div>
        <div class="detail_info">单曲: {{artistinfo.musicSize}} 专辑: {{artistinfo.albumSize}} MV: {{artistinfo.mvSize}}</div>
      </div>
    </div>
    <el-tabs 
      v-model="activeName"
                ...
                <part>
                   <div v-if="albumlist.length" class="recommend_list">
                    <div class="music_item"  v-for="(item,index) in albumlist" :key="index">
                        <Myitem :shadow="false" :imgurl="`${item.picUrl}?param=150y150`" :routemode="true" :router="{name:'albumsdetail',params:{id:item.id}}" class="item"></Myitem>
                        <Mytitle :routemode="true" :router="{name:'albumsdetail',params:{id:item.id}}" class="title">{{item.name}}</Mytitle>
            ...
            <part>
                <div v-if="mvlist.length" class="recommend_mv">
                    <div class="music_item"  v-for="(item,index) in mvlist" :key="index">
                        <Myitem :imgurl="`${item.imgurl}?param=300y140`" :routemode="true" :router="{name:'videodetail',params:{type:'mv',id:item.id}}" :number="item.playCount" class="item"></Myitem>
                        <Mytitle :routemode="true" :router="{name:'videodetail',params:{type:'mv',id:item.id}}" class="title">{{item.name}}</Mytitle>
                        <Mytitle v-if="item.artist" :routemode="true" :router="{name:'artistdetail',params:{id:item.artist.id}}" class="title" size="small" color="light">{{item.artist.name}}</Mytitle>
                    </div>
                </div>
                <el-empty v-else description="没有找到数据"></el-empty>
            </part>
...
import { computed, onActivated, onBeforeMount } from '@vue/runtime-core';
