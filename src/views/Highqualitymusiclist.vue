                    ...
                    <part class="type">
                        <div class="category_container">
                            <el-popover placement="bottom-start" :width="700" trigger="click">
                                ...
                                <div class="all_type">
                                    <span @click="handleClick('全部')" class="all_type_sort_child"
                                        :class="{ 'is-active': cat === '全部' }">全部分类</span>
                                </div>
                                <div class="pop_container_musiclist">
                                    <div class="pop_container_musiclist_sort_container" v-for="(item, index) in allcat"
                                        ...
                                        <span @click="handleClick(item)" class="pop_container_musiclist_sort_child"
                                            :class="{ 'is-active': item === cat }">{{ item }}</span>
                                    </div>
                                </div>
                            </el-popover>
                        </div>
                    ...
                    <div class="recommend">
                        <div class="item_container" v-for="(item, index) in highqualitylist" :key="index">
                            <myitem :routemode="true" :router="{ name: 'musiclistdetail', params: { id: item.id } }"
                                :imgurl="`${item.coverImgUrl}?param=130y130`" class="item"></myitem>
                            <div class="info">
                                <Mytitle class="title" :routemode="true"
                                    :router="{ name: 'musiclistdetail', params: { id: item.id } }">{{ item.name }}</Mytitle>
                                <Mytitle class="title" size="" color="light"
                                    :router="{ name: 'artistdetail', params: { id: item.creator.userId } }"
                                    :routemode="true">
                                    by:{{ item.creator.nickname }}</Mytitle>
