  ...
  <part title="最近播放" :issub="complete&&dataok" :subtitle="`最近${musiclist.total}首`">
    ...
    <el-table
        ...
        <el-table-column  show-overflow-tooltip label="音乐标题" min-width="160">
            <template v-slot="{row}">
              <span class="text_title" :class="{'playon':nowplaymusic&&row.data.id===nowplaymusic.id}">{{row.data.name}}</span>
            </template>
        </el-table-column>
        <el-table-column show-overflow-tooltip label="歌手" min-width="60">
            <template v-slot="{row}">
              <mytitle size="small" color="light" :class="{'playon':nowplaymusic&&row.data.id===nowplaymusic.id}" :routemode="true" :router="{name:'artistdetail',params:{id:artist.id}}" v-for="(artist,index) in row.data.ar" :key="index">{{index>0?'/':''}}{{artist.name}}</mytitle>
            </template>
        </el-table-column>
        <el-table-column show-overflow-tooltip min-width="60" label="播放时间">
            <template v-slot="{row}">
              <Timetransform class="sub_title" :class="{'playon':nowplaymusic&&row.data.id===nowplaymusic.id}" :time="row.playTime"/>
            </template>
        </el-table-column>
      </el-table>
