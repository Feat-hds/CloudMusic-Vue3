        ...
        <part title="收藏的视频" :issub="true" :subtitle="`(${total})`">
            <div v-if="submvlist.length" class="music_list">
                <div class="item_container"  v-for="(item,index) in submvlist" :key="index">
                    <Myitem :duration="item.durationms" :routemode="true" :router="{name:'videodetail',params:{id:item.vid,type:item.type?'video':'mv'}}" :imgurl="`${item.coverUrl}?param=240y150`" class="item" :number="item.playTime"></Myitem>
                    <Mytitle :routemode="true" :router="{name:'videodetail',params:{id:item.vid,type:item.type?'video':'mv'}}" class="title"><tags class="mv" v-if="!item.type" color="red">MV</tags>{{item.title}}</Mytitle>
                    <Mytitle size="small" color="light" :routemode="true" :router="item.type?{name:'userprofile',params:{id:user.userId}}:{name:'artistdetail',params:{id:user.userId}}" v-for="(user,i) in item.creator" :key="i" class="atist">{{item.type?'by ':''}}{{i>0?'/':''}}{{user.userName}}</Mytitle>
                </div>
            </div>
