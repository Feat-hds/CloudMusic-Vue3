        ...
        <part title="收藏的专辑" :issub="true" :subtitle="`(${total})`">
            ...
            >
                <el-table-column width="80">
                  <template v-slot="{row}">
                  <Myitem :shadow="false" :routemode="true" :router="{name:'albumsdetail',params:{id:row.id}}" :imgurl="`${row.picUrl}?param=60y60`" class="cover"></Myitem>
                  </template>
                </el-table-column>
                <el-table-column show-overflow-tooltip min-width="120">
                <template v-slot="{row}">
                    <mytitle :routemode="true" :router="{name:'albumsdetail',params:{id:row.id}}">{{row.name}}<span v-if="row.alias[0]" class="sub_info">({{row.alias[0]}})</span></mytitle>
                </template>
                </el-table-column>
                <el-table-column show-overflow-tooltip min-width="80">
                <template v-slot="{row}">
                    <mytitle color="light" size="small" :routemode="true" :router="{name:'artistdetail',params:{id:artist.id}}" v-for="(artist,i) in row.artists" :key="i">{{i>0?'/':''}}{{artist.name}}</mytitle>
                </template>
                </el-table-column>
