    ...
    <div v-if="complete && dataok">
      ...
      <el-table v-if="musiclist.length" :data="musiclist" style="width:100%;cursor:default" stripe lazy class="list"
        ...
        <el-table-column show-overflow-tooltip label="标题" min-width="160">
          <template v-slot="{ row }">
            <span class="text_title" :class="{ 'playon': nowplaymusic && row.id === nowplaymusic.id }"
              @click="test">{{ row.name }}<span v-if="row.tns" class="sub_info">({{ row.tns[0] }})</span></span>
        ...
        <el-table-column show-overflow-tooltip label="歌手" min-width="100">
          <template v-slot="{ row }">
            <mytitle v-for="(artist, i) in row.ar" :class="{ 'playon': nowplaymusic && row.id === nowplaymusic.id }"
              :routemode="true" :router="{ name: 'artistdetail', params: { id: artist.id } }" size="small" color="light" :key="i">
              {{ i > 0 ? '/' : '' }}{{ artist.name }}</mytitle>
          </template>
        </el-table-column>
        <el-table-column show-overflow-tooltip label="专辑" min-width="60">
          <template v-slot="{ row }">
            <mytitle size="small" :class="{ 'playon': nowplaymusic && row.id === nowplaymusic.id }" color="light"
        ...
        <el-table-column prop="time" show-overflow-tooltip min-width="40" label="时长">
          <template v-slot="{ row }">
            <duration class="table_time" :class="{ 'playon': nowplaymusic && row.id === nowplaymusic.id }" :time="row.dt">
            </duration>
          </template>
        </el-table-column>
      </el-table>
