  ...
  <div v-if="complete&&dataok" class="router_container">
    <div class="list_header">
      ...
      <div class="list_info">
        ...
        <div class="owner">
          <myitem :shadow="false" :hoveranim="false" :routemode="true" :router="{name:'userprofile',params:{id:radiodetail.dj.userId}}" :imgurl="`${radiodetail.dj.avatarUrl}?param=24y24`" class="avatar"/>
          <mytitle :routemode="true" :router="{name:'userprofile',params:{id:radiodetail.dj.userId}}" class="username">{{radiodetail.dj.nickname}}</mytitle>
        </div>
        <div class="button_group">
          <el-button @click="adddjandplay(null)" type="danger" size="small" icon="CaretRight" round>播放全部</el-button>
          <el-button v-if="issub" @click="subradio(radiodetail,0)" type="info" size="small" icon="FolderChecked" plain round>已收藏</el-button>
          <el-button v-else @click="subradio(radiodetail,1)" type="info" size="small" icon="FolderAdd" plain round>收藏</el-button>
        ...
        <el-tab-pane :label="`声音(${voicecount})`" name="list" class="tabs_list">  
          ...
          >
            <el-table-column width="80">
              <template v-slot="{row}">
              <Myitem @click="adddjandplay(row)" :shadow="false" :imgurl="`${row.coverUrl}?param=60y60`" class="cover"></Myitem>
              </template>
            </el-table-column>
            ...
            <el-table-column show-overflow-tooltip min-width="60">
              <template v-slot="{row}">
                <div class="text_table"><svg-icon iconClass="bx-play"/><playnumber :number="row.listenerCount"/></div>
              </template>
            </el-table-column>
            <el-table-column show-overflow-tooltip min-width="60">
              <template v-slot="{row}">
                <span class="text_table"><svg-icon iconClass="bx-like"></svg-icon><playnumber :number="row.likedCount" /></span>
              </template>
            </el-table-column>
            <el-table-column show-overflow-tooltip min-width="60">
              <template v-slot="{row}">
...
export default {
    ...
    setup(){
      ...
      let issub=computed(()=>{
        ...
        //     try{
        //         await getradiodetail(route.params.id)
        //         await getradioprogram(route.params.id,30,0);
        //         await getradiosubscriber(route.params.id)
        //         id.value=route.params.id
        //     }
        //     catch(err){
