  ...
  <div v-if="complete&&dataok" class="router_container">
    <div class="list_header">
      ...
      <div class="list_info">
        ...
        <div class="button_group">
          ...
          <el-button v-if="issub" @click="subalbum(musiclistinfo,2)" type="info" size="small" icon="FolderChecked" plain round>已收藏</el-button>
          <el-button v-else @click="subalbum(musiclistinfo,1)" type="info" size="small" icon="FolderAdd" plain round>收藏</el-button>

          <el-button @click="addall" type="info" size="small" icon="Plus" plain round>添加全部至播放列表</el-button>
        </div>
        <div class="owner">
          歌手:<mytitle :routemode="true" :router="{name:'artistdetail',params:{id:musiclistinfo.album.artist.id}}" class="username">{{musiclistinfo.album.artist.name}}</mytitle>
        </div>
        <div>时间:<timetransform class="created_time" :time="musiclistinfo.album.publishTime" /></div>
      </div>
    </div>
    <el-tabs 
      v-model="activeName"
    ...
    >
        <el-tab-pane label="歌曲列表" name="list" class="tabs_list">  
              <el-table
              v-if="musiclistinfo.songs.length"
        :data="musiclistinfo.songs"
        ...
        <el-table-column show-overflow-tooltip label="标题" min-width="160">
            <template v-slot="{row}">
              <span class="text_title" :class="{'playon':nowplaymusic&&row.id===nowplaymusic.id}" @click="test">{{row.name}}<span v-if="row.alia[0]" class="sub_info">({{row.alia[0]}})</span><tags class="row_tag" v-if="row.fee===1">VIP</tags><tags  class="row_tag" v-if="row.fee===8||row.fee===1">SQ</tags><tags  class="row_tag" v-if="row.mv!=0" :routemode="true" :router="{name:'videodetail',params:{id:row.mv,type:'mv'}}">MV</tags></span>
            </template>
        </el-table-column>
        <el-table-column show-overflow-tooltip label="歌手" min-width="100">
            <template v-slot="{row}">
            <mytitle :class="{'playon':nowplaymusic&&row.id===nowplaymusic.id}" v-for="(artist,i) in row.ar" :routemode="true" :router="{name:'artistdetail',params:{id:artist.id}}" size="small" color="light" :key="i">{{i>0?'/':''}}{{artist.name}}</mytitle>
            </template>
        </el-table-column>
        <el-table-column show-overflow-tooltip label="专辑" min-width="60">
            <template v-slot="{row}">
            <mytitle :class="{'playon':nowplaymusic&&row.id===nowplaymusic.id}" size="small" color="light" :routemode="true" :router="{name:'albumsdetail',params:{id:row.al.id}}">{{row.al.name}}</mytitle>
            </template>
        </el-table-column>
        <el-table-column prop="time" show-overflow-tooltip min-width="40" label="时长">
            <template v-slot="{row}">
            <duration :class="{'playon':nowplaymusic&&row.id===nowplaymusic.id}" class="table_time" :time="row.dt"></duration>
            </template>
        </el-table-column>
        </el-table>
...
import {reqAlbumdetail,reqAlbumdynamicdetail,reqAlbumcomment,reqSubalbumlist} from '@/api/music'
    ...
    setup(){
      ...
      async function getsubalbumlist(limit,offset){
          if(!isalbumlist.value&&islogin.value){
              ...
              // console.log(result);
              if(result.code===200){
                  // subalbumlist.value=result.data;
      ...
      // async function getalbumdynamic(id){
      //       let result=await reqAlbumdynamicdetail(id);
      //       // console.log(result);
      //       if(result.code===200){
      //         issub.value=result.isSub;
      //           // mvlist.value=result.data;
      //           return true;
      //       }
      //       else return Promise.reject();
          ...
          try{
            if(route.params.id!=id.value){
              ...
              if(route.params.id){
                ...
                // await getalbumdynamic(route.params.id);
