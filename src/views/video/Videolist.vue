                    ...
                    <el-popover placement="bottom-start" :width="700" trigger="click">
                        ...
                        <el-scrollbar class="scroll_bar_pop_container">
                            <div class="pop_container_musiclist">
                                ...
                                <div class="pop_container_musiclist_sort_container" v-for="(item, index) in grouplist"
                                    :key="index">
                                    <span @click="handleClick(item)" class="pop_container_musiclist_sort_child"
                                        :class="{ 'is-active': item.id === currentsort.id }">{{ item.name }}</span>
                                </div>
            ...
            <part>
                <div class="music_list">
                    <div class="item_container" v-for="(item, index) in videolist" :key="index">
                        <Myitem :duration="item.data.durationms" :routemode="true"
                            :router="{ name: 'videodetail', params: { id: item.data.vid, type: 'video' } }"
                            :imgurl="`${item.data.coverUrl}?param=240y150`" class="item" :number="item.data.playTime">
                        </Myitem>
                        <Mytitle :routemode="true"
                            :router="{ name: 'videodetail', params: { id: item.data.vid, type: 'video' } }" class="title">{{
                                item.data.title }}</Mytitle>
                        <Mytitle size="small" color="light" :routemode="true"
                            :router="{ name: 'userprofile', params: { id: item.data.creator.userId } }" class="atist">by
                            {{ item.data.creator.nickname }}</Mytitle>
                    </div>
                </div>
            </part>
        </div>
