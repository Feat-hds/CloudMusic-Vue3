        ...
        <part class="header">
            <div class="hotcat_container">
                <div  v-for="(item,index) in catlist" :key="index" class='sort_container'>
                    <span @click="handleClick(item)" class="sort_child" :class="{'is-active':item===currentsort}">{{item}}</span>
                </div>
            </div>
        </part>
            ...
            <div class="music_list">
                <div class="item_container"  v-for="(item,index) in newestmvlist" :key="index">
                    ...
                    <div class="artist_container">
                        <Mytitle size="small" color="light" :routemode="true" :router="{name:'artistdetail',params:{id:item.id}}" class="artist" v-for="(artist,i) in item.artists" :key="i">{{i>1?'/':''}}{{artist.name}}</Mytitle>
                    </div>
                </div>
            </div>
        </part>
        ...
        <part title="网易出品" :routemode="true" :router="{name:'allmv',params:{type:'网易出品',order:'最新',area:'全部'}}">
            <div class="music_list">
                <div class="item_container"  v-for="(item,index) in exclusivemvlist" :key="index">
                    ...
                    <Mytitle :routemode="true" :router="{name:'videodetail',params:{id:item.id,type:'mv'}}" class="title">{{item.name}}</Mytitle>
                    <div class="artist_container">
                        <Mytitle size="small" color="light" :routemode="true" :router="{name:'artistdetail',params:{id:item.id}}" class="atist" v-for="(artist,i) in item.artists" :key="i">{{i>1?'/':''}}{{artist.name}}</Mytitle>
                    </div>
                </div>
            </div>
        </part>
