                ...
                <div class="category_container">
                    <div class="hotcat_container">
                        ...
                        <div v-for="(item, index) in catlistarea" :key="index" class='sort_container'>
                            <span @click="handleClick(item, 1)" class="sort_child"
                                :class="{ 'is-active': item === currentarea }">{{ item }}</span>
                        </div>
                    </div>
                    <div class="hotcat_container">
                        <span class="sort_title">类型:</span>
                        <div v-for="(item, index) in catlisttype" :key="index" class='sort_container'>
                            <span @click="handleClick(item, 2)" class="sort_child"
                                :class="{ 'is-active': item === currenttype }">{{ item }}</span>
                        </div>
                    </div>
                    <div class="hotcat_container">
                        <span class="sort_title">排序:</span>
                        <div v-for="(item, index) in catlistorder" :key="index" class='sort_container'>
                            <span @click="handleClick(item, 3)" class="sort_child"
                                :class="{ 'is-active': item === currentorder }">{{ item }}</span>
                        </div>
                    </div>
                </div>
            </part>
                ...
                <div class="music_list">
                    <div class="item_container" v-for="(item, index) in mvlist" :key="index">
                        ...
                        <div class="artist_container">
                            <Mytitle size="small" color="light" v-for="(ar, i) in item.artists" :key="i" :routemode="true"
                                :router="{ name: 'artistdetail', params: { id: ar.id } }" class="artist">{{ i > 1 ? '/' : '' }}{{ ar.name }}
                            </Mytitle>
                        </div>
                    </div>
                </div>
