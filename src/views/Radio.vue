...
<el-scrollbar>
    ...
    <div v-if="complete&&dataok" class="recommend_container">
            <div class="header_card">
                <div class="card">
                    <mytitle color="light" :routemode="true" :router="{name:'radiotype',params:{type:cat.id,name:cat.name}}" class="cat" v-for="(cat,index) in radiocatlist" :key="index"><svg-icon class="icon" iconClass="bx-category"/>{{cat.name}}</mytitle>
                </div>
            </div>
            <part v-if="radiorec.length" title="推荐播客">
                <div class="recommend_list">
                    <div class="music_item"  v-for="(item,index) in radiorec" :key="index">
                        <Myitem :shadow="false" :imgurl="`${item.picUrl}?param=150y150`" :routemode="true" :router="{name:'radiodetail',params:{id:item.id}}" class="item"></Myitem>
                        <Mytitle :routemode="true" :router="{name:'radiodetail',params:{id:item.id}}" class="title"><tags class="tags" color="gray">{{item.secondCategory}}</tags>{{item.name}}</Mytitle>
                    </div>
                </div>
            </part>
            <part :routemode="true" :router="{name:'radiotype',params:{type:item.categoryId,name:item.categoryName}}" v-for="(item,index) in radiohotcat" :key="index" :title="item.categoryName">
                <div class="radio_type">
                    <div class="music_item"  v-for="(radio,i) in radiolist[index]" :key="i">
                        <Myitem :shadow="false"  :imgurl="`${radio.picUrl}?param=150y150`" :routemode="true" :router="{name:'radiodetail',params:{id:radio.id}}"  class="item"></Myitem>
                        <Mytitle class="title" :routemode="true" :router="{name:'radiodetail',params:{id:radio.id}}"><tags class="tags" color="gray">{{radio.secondCategory}}</tags>{{radio.name}}</Mytitle>
                    </div>
                </div>
            </part>
    </div>
...
import {reqRadioprivaterec,reqRadiorecommend,reqRadiocat,reqRadiocatrec,reqRadiohotcat} from '@/api/music'
